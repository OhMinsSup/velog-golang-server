...
오늘은 gorm 으로 작성한 미들웨어를 entgo 로 변경하는 작업을 진행했다. 중간에 서버가 실행이 안되는 이슈가 있있어는데
database/sql 을 사용해서 entgo 를 실행하면 sql driver가 없는다는 이슈가 있었는데, entgo 문서에
github.com/jackc/pgx/v4/stdlib 이 라이브러리를 사용하면 된다는 내용을 보고 수정했다.
...
오늘은 어제 있었던 버그들을 수정하고 태그 테이블을 생성해서 포스트 작성시 추가하는 로직을 작성하고 있다.
전에 비슷한 작업을 했었는데 이번에는 좀 더 golang 스러운 방법으로 작업을 할려고 한다.
무엇보다 전에 구현한 코드는 중복및 쓸데없는 작업도 같이 한 것 같아서 그 부분을 수정 할 예정이다.

### 오늘 한 일
- 태그 모델 생성
- 포스트 작성 API 이슈 처리 (urlSlug)
- 포스트 작성 API에서 태그 연결 (1)

## EP.16 포스트 작성 (3)

오늘은 포스트 작성하는 API 작업이 끝났다. 드디어 이제 업데이트하는 API, 상세 API를 만들면 대략적인 
포스트 작업이 끝나고 리스트까지 완성하면 되겠다
